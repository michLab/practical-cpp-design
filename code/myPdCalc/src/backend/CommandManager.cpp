...
class CommandManager::UndoRedoListStrategyVector
    : public CommandManager::CommandManagerImpl
{
  public:
...
{
    if (!undoRedoList_.empty()) {
        undoRedoList_.erase(undoRedoList_.begin() + cur_ + 1,
                            undoRedoList_.end());
    }
}

class CommandManager::UndoRedoListStrategy
    : public CommandManager::CommandManagerImpl
{
  public:
    UndoRedoListStrategy()
        : undoSize_{0}, redoSize_{0}, cur_{undoRedoList_.end()}
    {
    }
...
size_t CommandManager::getUndoSize() const { return pimpl_->getUndoSize(); }

size_t CommandManager::getRedoSize() const { return pimpl_->getRedoSize(); }
...
void CommandManager::undo() { pimpl_->undo(); }

void CommandManager::redo() { pimpl_->redo(); }
}  // namespace pdCalc