...
const char* StackEventData::Message(StackEventData::ErrorConditions ec)
{
    switch (ec) {
        ...
        case ErrorConditions::TooFewArguments:
            return "Need at least two stack elemenst to swap top";
        default:
            return "Unknown error";
    }
}

const char* StackEventData::message() const { return Message(err_); }
...
double Stack::StackImpl::pop(bool suppressChangeEvent)
{
    if (stack_.empty()) {
        parent_.raise(Stack::StackError,
                      std::make_shared<StackEventData>(
                          StackEventData::ErrorConditions::Empty));
        throw Exception(
            StackEventData::Message(StackEventData::ErrorConditions::Empty));
    } else {
...
void Stack::StackImpl::swapTop()
{
    if (stack_.size() < 2) {
        parent_.raise(Stack::StackError,
                      std::make_shared<StackEventData>(
                          StackEventData::ErrorConditions::TooFewArguments));
        throw Exception(StackEventData::Message(
            StackEventData::ErrorConditions::TooFewArguments));
    } else {
...
double Stack::StackImpl::top() const { return stack_.back(); }
...
void Stack::swapTop() { pimpl_->swapTop(); }
...
std::vector<double> Stack::getElements(size_t n)
{
    return pimpl_->getElements(n);
}

size_t Stack::size() const { return pimpl_->size(); };

void Stack::clear() const { pimpl_->clear(); }
